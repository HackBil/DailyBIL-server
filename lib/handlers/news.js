...
module.exports.post = function(req, res, next) {
  ...
  async.waterfall([
    function checkIntegrity(cb) {
      ...
      if(!req.params.token || !req.params.url || !req.params.title || !req.params.categories) {
        return cb(new restify.InvalidArgumentError("Missing argument: need a token, an url, a title and a least 1 category"));
      }
